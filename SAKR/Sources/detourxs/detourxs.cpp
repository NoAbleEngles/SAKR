...
BOOL DetourXS::Create(const LPVOID lpFuncOrig, const LPVOID lpFuncDetour)
{
	...
	if(VirtualProtect(m_trampoline.data(), m_trampoline.size(), F4SE::WinAPI::PAGE_EXECUTE_READWRITE, &dwProt) == FALSE) { return FALSE; }

	// Write jump from orig to detour
	if(VirtualProtect(m_lpFuncOrig, m_detourLen, F4SE::WinAPI::PAGE_EXECUTE_READWRITE, &dwProt) == FALSE) { return FALSE; }
...
{
	DWORD dwProt;
	VirtualProtect(m_lpFuncOrig, m_detourLen, F4SE::WinAPI::PAGE_EXECUTE_READWRITE, &dwProt);
...
LPVOID DetourXS::RecurseJumps(LPVOID lpAddr)
{
	...
	#else
		lpDest = reinterpret_cast<LPVOID>( *reinterpret_cast<PDWORD>(lpbAddr + 1) + reinterpret_cast<DWORD>(lpbAddr) + relativeJmpSize 
			+ (reinterpret_cast<DWORD_PTR>(F4SE::WinAPI::GetModuleHandle(static_cast<const wchar_t*>(nullptr))) & 0xFFFFFFFF00000000) );
	#endif
